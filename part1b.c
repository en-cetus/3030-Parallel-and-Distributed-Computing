...
/*---------------------------------------------------------------------
 * Function: Compute_local_force_stage0
 * Purpose:  Compute initial forces between particles owned locally (Stage 0).
 *           Applies self-interaction guard (i != k).
 */
void Compute_local_force_stage0(double masses[], vect_t loc_pos[],
                               vect_t loc_forces[], int loc_n, int my_rank) {
   int i, k;
...
#ifdef DEBUG_TEST
/*---------------------------------------------------------------------
 * Function: Run_Stage0_Unit_Test
 * Purpose:  Unit test for Stage 0 force computation using 2 dummy particles.
 */
void Run_Stage0_Unit_Test(int my_rank) {
   if (my_rank == 0) {
      printf("\n================ [UNIT TEST RUNNING] ================\n");
      ...
      if (error < 1.0e24) {
         ...
         printf("[FAIL] Stage 0 Force Test FAILED! Expected = %e, Got = %e\n", expected_fx, test_forces[0][X]);
      }
      printf("=====================================================\n\n");
   }
}
